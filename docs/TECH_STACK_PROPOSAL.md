...
확정 정책은 [요구사항](REQUIREMENTS.md)을 따른다. 데이터 접근 방식은 D73, 나머지 기술 스택은 D74로 확정했다. 잠금 적용 방침은 D75를 따르며 주요 키·데이터 범위·JPA 관계·삭제·스키마 관리 방침은 D76~D78로 확정했다. 세부 컬럼 타입·길이·생성 구문은 검토 중이다. 실행 기반 구성과 검증 결과는 별도 환경 PR에서 기록한다.
...
| 빌드 | Gradle Kotlin DSL, Java 25와 호환되는 Gradle 9.x | 정확한 Wrapper 버전과 배포 체크섬은 실행 환경 PR에서 검증·고정 |
