...
개설 강좌의 수업 시간과 강의 담당 연결은 각각 최소 1개이다(D77). 이 최소 개수는 단순 FK만으로 보장되지 않으며 데이터 구성은 초기 데이터 생성·검증 단계에서 확인한다. 현재 V1~V9와 엔티티·Repository의 검증 범위는 [DB 검증 기록](DB_SETUP.md)을 따른다.
