...
Docker Desktop, JDK 25를 준비한다. `.env.example`을 `.env`로 복사해 DB 비밀번호와 개발용 학생 초기 비밀번호를 지정한다. `.env`는 Git에 포함하지 않는다.
...
IntelliJ에서는 Gradle bootRun 실행 구성의 환경변수에 DB_PASSWORD와 INITIAL_STUDENT_PASSWORD를 등록한다. JDBC 기본 주소는 localhost:5432/course_registration, 계정은 course_app이다. 데이터가 이미 정상적으로 생성된 이후에는 초기 학생 비밀번호 없이도 재실행할 수 있다.
...
전체 DB 통합 테스트는 Testcontainers의 별도 PostgreSQL 18.6을 사용한다. 개발 DB 환경변수나 개발 DB 초기화가 필요하지 않다. Flyway V1~V10이 스키마를 생성하고 Hibernate ddl-auto=validate가 매핑을 검사한다. CHECK·UNIQUE·FK는 실제 PostgreSQL 저장 실패 테스트로 검증한다.
