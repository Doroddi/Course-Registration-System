...
강좌 목록은 먼저 수강신청한 순서로 반환한다(D63). 취소 후 재신청한 강좌는 새 신청 순서에 배치한다. 전체 강좌 조회의 강좌명·ID 정렬과 구분한다. 대상 학생·학기의 신청을 증가 enrollment_id 오름차순으로 반환한다(D66). 같은 학생의 동시 요청은 학생 잠금 안에서 성공적으로 저장·커밋한 순서이며 HTTP 도착 순서와는 구분한다.
