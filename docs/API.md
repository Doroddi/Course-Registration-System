...
예를 들어 size=101이고 학과 ID도 존재하지 않으면 학과를 조회하기 전에 400으로 응답한다. 인증 단계의 처리 순서는 별도 설계 대상이다.

| 상황 | HTTP 상태 | code |
|---|---|---|
| 토큰 누락·위조·만료 | 401 Unauthorized | 공통 인증 오류 본문에서 확정 예정 |
...
400·404 오류 본문은 code와 message를 제공한다. 401 오류의 code·message는 공통 인증 설계에서 확정한다. code는 프로그램이 오류 종류를 구분하는 값이며 message는 잘못된 항목과 허용 범위 또는 형식을 설명한다.
...
- 인증 오류와 입력 오류가 동시에 발생할 때의 우선순위 및 공통 401 오류 본문
